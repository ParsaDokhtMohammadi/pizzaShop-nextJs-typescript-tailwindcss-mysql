...
const Card = async({ data , inCart}: { data: ICartItem , inCart : boolean}) => {
  ...
  return (
    <div className='flex flex-col   gap-4  rounded-2xl  relative items-center'>
        <Image src={data.imageURL || ""} alt={data.name} width={300} height={300}   className='w-[150px] h-[140px] relative top-20 z-10 '/>
        <div className='flex flex-col justify-between  h-[200px] w-[225px] rounded-2xl bg-bgPrimary px-5 pb-3 '>
